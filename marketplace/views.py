    ...
    def get(self, request):
        ...
        if Shares.shares.get_user_companies(user):
            try:
                ...
                form = SellSharesForm(user, initial={'company': company, 'shares': shares})
            except ValueError:
    ...
    def post(self, request):
        ...
        if Shares.shares.get_user_companies(user):
            ...
            if form.is_valid():
                ...
                user_shares = Shares.shares.get(company=company, user=user)
                if user_shares.count < shares:
                    context = {'form': form, 'errors': ['Недостаточно акций в портфеле']}
                    return render(request, self.template, context)
                ...
                lots = Lot.lots.filter(company=company, price=price, user=user).all()
                context = {'form': form, 'messages': ['Акции успешно выставлены на продажу.']}
                if lots:
                    lots[0].count += shares
                    lots[0].save()
                    context['messages'].append('Акции были добавлены к уже существующему лоту.')
                else:
                    Lot.lots.create(company=company, count=shares, price=price, user=user)
...
class BuySharesView(View):
    ...
    def get(self, request):
        ...
        try:
            ...
            form = self.form(
                initial={'seller': seller, 'company': company, 'price': price, 'shares': shares}
            )
            context = {
                'form': form,
    ...
    def post(self, request):
        ...
        try:
            ...
            context = {'form': form, 'errors': ['Неверные данные формы.']}
            return render(request, self.template, context)
        except MultiValueDictKeyError:
            context = {'form': form, 'errors': ['Неверные данные формы.']}
            return render(request, self.template, context)
        except ObjectDoesNotExist:
            context = {'form': form, 'errors': ['Предложение недействительно.']}
            return render(request, self.template, context)

        if form.is_valid():
            shares = int(form.cleaned_data['shares'])

            seller_shares = Lot.lots.filter(company=company, user=seller, price=price).all()
            ...
            user_shares = Shares.shares.filter(company=company, user=user).all()

            if user_shares:
                user_shares[0].count += shares
                user_shares[0].save()
            else:
                Shares.shares.create(company=company, count=shares, user=user)
            ...
            context = {'form': form, 'messages': ['Акции успешно приобретены']}
